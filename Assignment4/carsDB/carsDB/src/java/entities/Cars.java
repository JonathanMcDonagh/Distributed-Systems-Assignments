...
@NamedQueries({
    ...
    /* Custom Query To Find By CarID and Year, Query To Find By Make and Model */
    , @NamedQuery(name = "Cars.findByCarIdAndYear", query = "SELECT c FROM Cars c WHERE c.carID = :carID AND c.cYear = :cYear")
    , @NamedQuery(name = "Cars.findByMakeAndModel", query = "SELECT c FROM Cars c WHERE c.cMake = :cMake AND c.cModel = :cModel")
})
